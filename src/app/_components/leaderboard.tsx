...
export const CountryVoteUi: React.FC<{
  countryVotes: CountryInfoDb;
  index: number;
}> = ({ countryVotes, index }) => {
  const countryName =
    countryLookUp.byInternet(countryVotes?.countrySymbol || "")?.country || "";
  let medalSrcImg = "";
  switch (index) {
    case 0:
  ...
  return (
    <div
      className={cn(
        "w-full flex relative countryLeaderBoardItem   items-center px-1  lg:px-4",
        { "border-t-0 border-white": index === 0 }
      )}
    >
      <div className="flex  items-center w-[30%] gap-1   lg:gap-3 ">
        <div className="min-w-[30px] max-w-[30px] lg:min-w-[50px] lg:max-w-[50px] flex items-center justify-center relative">
          {medalSrcImg ? (
            <img src={medalSrcImg} alt="medal" className="w-full " />
          ) : (
            <h3 className="text-lg lg:text-2xl  block  font-bold ">
              #{index + 1}
            </h3>
          )}
        </div>
        <img
          src={`/flags/${countryVotes.countrySymbol
            .trim()
            .toLocaleLowerCase()}.png`}
          className="w-[30px] lg:w-[74px] border border-black "
        />
        <h4
          style={{ color: "black", margin: 0, textAlign: "start" }}
          className="H6 "
        >
          {countryName}
        </h4>
...
export const LeaderBoard: React.FC<{
...
}> = ({ ipAddress, countriesVotes: initialCountriesVotes }) => {
  ...
  const userCountry = countriesVotes.find(
    (country) => country.countrySymbol === countrySymbol
  );
  totalCountryVotes = userCountry
    ? userCountry?.biden + userCountry?.trump
    : undefined;
  ...
  const openLeaderBoard = () => {
    ...
    if (leaderboardContainer.current) {
      const height =
        leaderboardContainer.current.getBoundingClientRect().height;
      openLeaderboardAnimation.start({
        y: -height,
  ...
  const greatestCountry: CountryInfoDb | undefined = countriesVotes.length
    ? countriesVotes?.reduce((acc, countryVotes) => {
        if (acc.biden + acc.trump < countryVotes.biden + countryVotes.trump)
          return countryVotes;
        else return acc;
      })
    : undefined;
  return (
    <motion.div
      initial={{ y: 0 }}
      animate={openLeaderboardAnimation}
      className={cn(
        "  absolute bottom-0  w-full  mt-4 h-[56px] lg:h-[65px]    flex flex-col",
    ...
    >
      <div className="px-3 lg:px-8 py-2  bg-white shadow-2xl shadow-white border-b-2 border-white bg-opacity-55 backdrop-blur rounded-t-lg   flex items-center justify-between">
        {greatestCountry ? (
          <div className="flex gap-1 items-center  ">
            ...
            <img
              src={`/flags/${greatestCountry.countrySymbol
                ?.trim()
                .toLowerCase()}.png`}
              className="w-[30px] lg:w-[50px]"
            />
            <h6 className="H7" style={{ margin: 0, color: "black" }}>
              {(greatestCountry.trump + greatestCountry.biden).toLocaleString()}
            </h6>{" "}
          </div>
        ) : (
        ...
        <div className="flex gap-1 lg:gap-4 items-center  ">
          ...
          />
          <h6 className="H7" style={{ margin: 0, color: "black" }}>
            {totalCountryVotes?.toLocaleString() || 0}
          </h6>
          {leaderboardOpened ? (
            <img
              onClick={closeLeaderBoard}
              src="/icons/dropDown.svg "
              className="w-[25px] rotate-180 cursor-pointer"
            />
          ) : (
            <div className="relative">
              <img
                className="absolute lg:-left-10 -top-14 -left-5 lg:-top-20 min-w-[70px] lg:min-w-[100px]  z-30  min-h-[100]"
                src="/clickAnimation.gif"
              />{" "}
              <img
                onClick={openLeaderBoard}
                src="/icons/dropDown.svg "
                className="w-[25px]  z-50 relative cursor-pointer"
              />
            </div>
          )}
        </div>
      ...
      <div
        ...
        className="flex max-h-[44vh] lg:max-h-[48vh] leaderBoardScrollbar
        ...
        {countriesVotes
          .sort(
            (countryB, countryA) =>
              countryA.trump +
              countryA.biden -
              (countryB.trump + countryB.biden)
          )
          .map((countryVote, index) => (
            <CountryVoteUi
              key={countryVote.countrySymbol}
              index={index}
