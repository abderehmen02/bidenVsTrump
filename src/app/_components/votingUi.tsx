...
export const VotingUi = () => {
  ...
  const [trumpVoteAnimations, setTrumpVoteAnimations] = useState<
    { element: React.ReactNode; key: number }[]
  >([]);
  const [bidenVoteAnimations, setBidenVoteAnimations] = useState<
    { element: React.ReactNode; key: number }[]
  >([]);
  ...
  const handleTrumpClick = () => {
    ...
    setTrumpVoteAnimations((animations) => [
      ...animations,
      {
        element: (
          <motion.h4
            style={{ zIndex: 100 }}
            className={cn("H1 absolute top-10  left-0 text-white")}
            transition={{ duration: 1 }}
            initial={{ y: -300, zIndex: 50, opacity: 1 }}
            animate={{ y: 0, zIndex: -50, opacity: [1, 1, 0.5, 0] }}
          >
            +1
          </motion.h4>
        ),
        key: trumpAnimationsKey.current,
      },
    ]);
  ...
  const handleBidenClick = () => {
    ...
    setBidenVoteAnimations((animations) => [
      ...animations,
      {
        element: (
          <motion.h4
            className={cn("H1 absolute top-10 right-0 text-white")}
            transition={{ duration: 1 }}
            initial={{ y: -300, zIndex: 50, opacity: 1 }}
            animate={{ y: 0, zIndex: -50, opacity: [1, 1, 0.5, 0] }}
          >
        ...
        key: bidenAnimationsKey.current,
      },
    ]);
  };
  ...
  return (
    <div className="flex relative  justify-between lg:gap-36  lg:px-8 w-full  z-0 lg:mt-1 ">
      <div className="flex z-10 relative   flex-col items-center lg:gap-0 gap-0">
        ...
        <div className="relative flex items-center justify-center  overflow-visible h-fit ">
          ...
          <img
            ...
            src="/animationPic.gif"
            className={cn(
              "hidden lg:block absolute  z-10 min-w-[230px]   lg:min-w-[500px] ",
              { invisible: !votingBiden }
            )}
          />
          <img
            className="w-[380px] lg:w-[200px] z-20  cursor-pointer"
            onClick={handleBidenClick}
            src="/harisImg.png"
          />
        </div>
      </div>
      <div className="flex flex-col pt-5  w-[300px]  h-fit  gap-2 lg:gap-4 items-center justify-end lg:justify-start xl:justify-between ">
        <div className="z-0  absolute top-1/2 flex  justify-center left-1/2 w-7/12 -translate-x-1/2 -translate-y-1/2">
          <img
            src="/vsBg.png"
            className="absolute z-[1]  lg:hidden opacity-70  top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full"
          />
          <img
            src="/vs.png"
            className="z-[2] w-[120px]  lg:w-[140px] -translate-y-12 lg:-translate-y-28"
          />
        </div>
      ...
      <div className="flex z-10 relative  flex-col items-center lg:gap-0 gap-0">
        ...
        <img
          className="w-[300px] lg:w-[250px] relative z-40 "
          src="/trumpText.png"
        />
        <div className="relative flex items-center justify-center  overflow-visible h-fit ">
          {votingTrump && (
            <img
              src="/animationPic.gif"
              className="absolute  z-10 min-w-[230px]  lg:min-w-[500px]  "
            />
          )}
          <img
            className="w-[380px] lg:w-[200px] relative z-20 cursor-pointer"
